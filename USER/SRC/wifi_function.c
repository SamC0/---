...
bool ESP8266_BuildAP ( char * pSSID, char * pPassWord, char * enunPsdMode )
{
	...
	return ESP8266_Cmd ( cCmd, "OK", 0, 1500 );
...
void ESP8266_ReceiveString ( void )
{
	u16 i;
	delay_ms(200);//???????!!!!!!!!!!
//	Rdata_len=strEsp8266_Fram_Record .InfBit .FramLength;
	for(i=0;i<200;i++)
	{
		Rdata[i] = strEsp8266_Fram_Record .Data_RX_BUF[i];
		//if(Rdata[i]==0xcc) break;
		Rdata_len++;
	}
...
//	ESP8266_Cmd ( "AT+CWLAP", "OK", 0, 5000 );
	...
	while ( ! ESP8266_JoinAP ( Wifi.name,Wifi.password) );
...
//	ESP8266_Cmd ( "AT", "OK", NULL, 200 );
	...
	do
	{
		...
		sprintf(a,"AT+CIPSTART=\"%s\",\"%s\",%s\r\n","TCP",Wifi.ip,Wifi.port);
		ESP8266_Usart ( "%s\r\n", a );	
		delay_ms ( 1000);                 //延时	
...
//{ 	
//	if(Rdata[0] == 0x07)
//		USART2_Sendstring(Data ,13);
//	else
//		USART2_Sendstring(Data ,411);
//	strEsp8266_Fram_Record .InfBit .FramLength = 0;
//	USART3_Sendstring(End_send,3);
//}	
...
void login(u8 usr_Id)
{
	...
	ESP8266_SendString ( ENABLE, "Host: 192.168.1.107:1000\r\n",      NULL, Single_ID );//以下为请求头
